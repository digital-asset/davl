...
export const makeVacation = <T extends object>(
  ...
  { employeeRole: { employee, boss }, fromDate, toDate }: v4.Vacation,
...
export const prettyRequests = (
  requestContracts: readonly CreateEvent<v4.VacationRequest>[],
): Vacation[] => {
  const requests: Vacation[] = requestContracts.map(({ contractId, payload }) =>
    makeVacation(contractId, payload.vacation),
  );
...
export const splitVacations = (
  vacationContracts: readonly CreateEvent<v4.Vacation>[],
) => {
  const today = moment().format("YYYY-MM-DD");
  const vacations = vacationContracts.map(vacation =>
    makeVacation(vacation.contractId, vacation.payload),
  );
