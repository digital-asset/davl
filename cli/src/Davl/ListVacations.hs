...
data ListAsBoss = ListAsBoss { list :: [Vacation] } -- *not* group by Employee
data ListAsEmployee = ListAsEmployee { list :: [Vacation] } -- *not* grouped by Boss
...
listAsBoss whoami state = do
    let list = vacationsAsBoss whoami state
    ListAsBoss {list}

listAsEmployee :: Party -> State -> ListAsEmployee
listAsEmployee whoami state = do
    let list = vacationsAsEmployee whoami state
    ListAsEmployee {list}
